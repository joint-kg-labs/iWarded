...
import org.vadalog.iwarded.generator.ModelGenerator;
 ...
 * It is employed as a benchmarking system for Warded Datalog+/- reasoning.
 * 
 ...
 * Each program is written in a .vada file and is accompanied by a set of generated .csv files as data sources.
 * 
...
public final class IWardedGenerator {
	...
	/* path to the generated program and data files */
	private String programPath = "generatedPrograms/";
	...
	private void presentParameters() {
		...
		System.out.println("- Number of Input Predicates:");
		System.out.println("the total number of edb predicates with the @input annotation and the .CSV dataset");
		System.out.println("[integer >= 1]");
		System.out.println();

		System.out.println("- Number of Output Predicates:");
		System.out.println("the total number of idb predicates with the @output annotation");
		System.out.println("[integer >= 0]");
		System.out.println();

		System.out.println("- Average number of Variables per Predicate: ");
		System.out.println("the average number of variables as arguments of each predicate");
		System.out.println("[integer >= 1]");
		System.out.println();

		System.out.println("- Variance of Variables per Predicate: ");
		System.out.println("the variance for the number of variables as arguments of each predicate");
		...
		System.out.println("the average number of existential quantifiers in each head of each existential rule");
		System.out.println("[integer >= 0]");
		System.out.println();

		System.out.println("- Variance of Existentially Quantified Variables per Rule: ");
		System.out.println("the variance for the number existential quantifiers in each head of each existential rule");
		System.out.println("[float >= 0]");
		System.out.println();

		System.out.println("- Number of Dangerous Rules: ");
		System.out.println("the total number of rules with dangerous variables");
		System.out.println("[integer >= 0]");
		System.out.println();
		...
		System.out.println("- Number of Input-Output Sequences: ");
		System.out.println("the total number of input-output sequences");
		System.out.println("[integer >= 0]");
		System.out.println();

		System.out.println("- Average Length of Input-Output Sequences: ");
		System.out.println("the average number of maximum steps in the chase per output atom");
		System.out.println("[integer >= 0]");
		System.out.println();
		...
		System.out.println("- Number of Left-Right Join Recursions: ");
		...
		System.out.println("- Shyness of the Program: ");
		System.out.println("whether for each rule the harmful join variables and dangerous variables are affected by distinct existentials");
		System.out.println("[boolean true/false]");
		System.out.println();
	...
	private void createModelFromInputArgs(LineReader reader){
		...
		System.out.print("Number of Input Predicates: ");
		parameters.add(reader.readLine());
		System.out.print("Number of Output Predicates: ");
		parameters.add(reader.readLine());
		System.out.print("Average number of Variables per Predicate: ");
		parameters.add(reader.readLine());
		System.out.print("Variance of Variables per Predicate: ");
		parameters.add(reader.readLine());
		...
		parameters.add(reader.readLine());
		System.out.print("Number of Input-Output Sequences: ");
		parameters.add(reader.readLine());
		System.out.print("Average Length for Input-Output Sequence: ");
		parameters.add(reader.readLine());
		...
		parameters.add(reader.readLine());
		System.out.print("Number of Left-Right Join Recursions: ");
		parameters.add(reader.readLine());
		...
		parameters.add(reader.readLine());
		System.out.print("Shyness of the Program: ");
		parameters.add(reader.readLine());
		...
		try{
			...
			fstream = new FileWriter(path + "/" + programName + ".vada");
	...
	public void createModelFromConfigScenario(String configPath) throws Exception {
		...
		System.out.println("\n=====Building Vadalog Program=====");
		List<String> parameters = new ArrayList<>();
		parameters.add(p.getProperty("NumberOfInputPredicates"));
		parameters.add(p.getProperty("NumberOfOutputPredicates"));
		parameters.add(p.getProperty("AverageVarsInPredicate"));
		parameters.add(p.getProperty("VarianceVarsInPredicate"));
		...
		parameters.add(p.getProperty("NumberOfInputOutputSequences"));
		parameters.add(p.getProperty("AverageInputOutputSequenceLength"));
		...
		parameters.add(p.getProperty("NumberOfLeftRightJoinRecursions"));
		...
		parameters.add(p.getProperty("IsShy"));
		String programName = p.getProperty("ProgramName");
		parameters.add(programName);
	...
	private void createFolder(String programName) {
		...
		try {
			Files.createDirectories(Paths.get(file.getPath() + "/inputCsv"));
			Files.createDirectories(Paths.get(file.getPath() + "/outputCsv"));
		} 
		catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) throws IOException {
		...
		/*otherwise, generate single program based on input configuration scenario*/
			...
			options.addOption("a", "args", false, "Build Program from Input Parameters");
			options.addOption("c", "config", true, "[path-to-configScenarioFile.txt] Build Program from Configuration Scenario");
			options.addOption("d", "docs", false, "Describe Input Parameters from Documentation");
			options.addOption("e", "exit", false, "Close iWarded");
